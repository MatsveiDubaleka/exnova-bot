...
const TRACKING_ID = 'G-X29Y487SBZ';

ReactGA.initialize(TRACKING_ID);
export default function App() {
  const [lang, setLang] = useState<LangsType>('en');
  const [step, setStep] = useState<number>(0);
  const gaEventTracker = useAnalyticsEventTracker('Test Exnova Bot');
  ...
  return (
    <div className='relative flex flex-col items-center justify-between w-full h-full text-white bg-cover bg-[#141822] '>
        ...
        {step === 1 && (
          <div className='mt-2 relative w-full h-full flex flex-col items-center justify-center max-w-[320px] text-center gap-2'>
            <h2 className='font-bold text-center text-2xl mx-[25px] z-20'>
              Powerful platform forsmart investing
            </h2>
            <p className='text-[#C0C1C5] z-20 text-center'>
              Invest in slices of the companies, currencies and commodities at
              the touch of a button — securely, from the device of your choice.
            </p>
          </div>
        )}
        {step === 2 && (
      ...
      {step === 0 && (
        <div className='mt-[-10vh] flex items-center justify-center h-full w-full'>
          <img src='/logo.svg' alt='logotype' width={132} height={34} />
        </div>
      )}
